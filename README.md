...
### Если плагин не собирает каталог:
1. **Перезапустите Figma** - плагин может кэшироваться
2. **Проверьте консоль** (View → Developer → Show Console)
3. **Убедитесь в наличии переменных** в файле
4. **Переменные должны быть локальными** (не из библиотеки)

### Отладка:
- Откройте консоль разработчика в Figma
- Проверьте логи при нажатии "Собрать каталог"
- Убедитесь, что в файле есть переменные
