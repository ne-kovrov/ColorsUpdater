  ...
  console.log('Тип сообщения:', msg.type);
    ...
    if (msg.type === 'collect-catalog') {
        ...
        try {
            ...
            figma.ui.postMessage({ type: 'error', detail: 'Не удалось собрать каталог: ' + ((e === null || e === void 0 ? void 0 : e.message) || String(e)) });
        }
        return;
    }
    if (msg.type === 'run') {
        try {
            const mapping = parseMappingCsv(msg.mappingCsv);
            const catalog = parseCatalog(msg.catalogCsvOrJson);
            ...
            figma.ui.postMessage({
                type: 'error',
                detail: 'Критическая ошибка: ' + ((e === null || e === void 0 ? void 0 : e.message) || String(e))
            });
        }
        finally {
            figma.closePlugin();
...
function sanitize(s) { return s.replace(/^\uFEFF/, '').trim(); }
...
function parseMappingCsv(csv) {
    ...
    for (const r of data) {
        ...
        if (styleName && variableName)
            res.push({ styleName, variableName, collectionName });
    }
    return res;
}
function parseCatalog(text) {
    ...
    const hasHeader = ['name', 'key', 'id', 'collection', 'collectionname', 'type'].some((h) => header.includes(h));
...
async function buildCatalogCsvFromThisFile() {
    const vars = await figma.variables.getLocalVariablesAsync();
    const collections = await figma.variables.getLocalVariableCollectionsAsync();
    const colsById = new Map(collections.map((c) => [c.id, c]));
    ...
    for (const v of vars) {
        ...
        const collectionName = (col === null || col === void 0 ? void 0 : col.name) || '';
        const type = v.resolvedType || v.type || '';
        lines.push([csvCell(v.name), csvCell(v.key || ''), csvCell(v.id), csvCell(collectionName), csvCell(String(type))].join(','));
    }
    const result = lines.join('\n');
    console.log(`Результат каталога: ${result.length} символов`);
    return result;
}
function csvCell(s) { const t = (s || ''); if (/[",\n]/.test(t))
    return '"' + t.replace(/"/g, '""') + '"'; return t; }
async function run(mapping, catalog) {
    const report = [];
    if (!mapping.length) {
    ...
    for (const v of localVars) {
        const arr = localByName.get(v.name) || [];
        arr.push(v);
        localByName.set(v.name, arr);
    }
    // Индексация каталога
    const catalogByName = new Map();
    for (const row of catalog) {
        const arr = catalogByName.get(row.name) || [];
        arr.push(row);
        catalogByName.set(row.name, arr);
    }
    // Кэш импортов по key/id
    const importedByKey = new Map();
    const importedById = new Map();
    async function resolveVariableByName(name, preferCollection) {
        ...
        if (pickLocal && isColorVar(pickLocal))
            return pickLocal;
        // 2) из каталога по имени (+ колллекция)
        const rows = catalogByName.get(name) || [];
        let row = rows.find(r => preferCollection ? (r.collectionName === preferCollection) : true) || rows[0];
        if (!row)
            return null;
        // key приоритетнее
        if (row.key && 'importVariableByKeyAsync' in figma.variables && typeof figma.variables.importVariableByKeyAsync === 'function') {
            const cached = importedByKey.get(row.key);
            if (cached)
                return isColorVar(cached) ? cached : null;
            try {
                const v = await figma.variables.importVariableByKeyAsync(row.key);
    ...
    for (const node of nodes) {
        ...
        for (const styleType of styleTypes) {
            if (styleType.prop in node) {
                const sid = node[styleType.prop];
                if (sid && sid !== figma.mixed) {
                    processedStyles++;
                    const style = await figma.getStyleByIdAsync(sid);
                    const styleName = style === null || style === void 0 ? void 0 : style.name;
                    if (styleName) {
                        ...
                        if (row) {
                            ...
                            if (!v) {
                                ...
                                if (styleType.binder === 'effects') {
                                    ok = bindVariableToEffects(node, v.id, report);
                                }
                                else {
                                    ok = bindVariableToPaints(node, v.id, styleType.binder, report);
                                }
                                if (ok) {
                                    successfulReplacements++;
                                    console.log(`Успешно привязана переменная к ${styleType.typeName}`);
                                    await node[styleType.setter]('');
                                }
                                else {
...
async function collectionNameOf(v) {
    try {
        const col = await figma.variables.getVariableCollectionByIdAsync(v.variableCollectionId);
        return col === null || col === void 0 ? void 0 : col.name;
    }
    catch (_a) {
        return undefined;
    }
...
function bindVariableToPaints(node, variableId, prop, report) {
    ...
    for (const p of paints) {
        if (p.type === 'SOLID') {
            ...
            p.boundVariables.color = { type: 'VARIABLE_ALIAS', id: variableId };
            changed = true;
        }
    }
...
function bindVariableToEffects(node, variableId, report) {
    if (!('effects' in node))
        return false;
    ...
    for (const e of effects) {
        if (e.type === 'DROP_SHADOW' || e.type === 'INNER_SHADOW') {
            ;
            e.boundVariables = e.boundVariables || {};
            e.boundVariables.color = { type: 'VARIABLE_ALIAS', id: variableId };
            changed = true;
        }
    }
    if (changed)
        ...
        report.push({ kind: 'warn', nodeName: node.name, nodeId: node.id, detail: 'Не удалось привязать переменную к эффектам (нет теней с цветом)' });
    return changed;
}
async function makeReportFrame(items) {
...
function formatReport(items) {
    if (!items.length)
        return 'Готово. Все стили на текущей странице успешно заменены на переменные.';
    const lines = [];
